...
import { CheckCircle2, ClipboardCheck, History, Loader2, Pill, Play, Save, Trash2 } from 'lucide-react';
import {
...
import { Alert, Button, ConfirmDialog, EmptyState, Field, PageHeader, Panel, StatusBadge } from '../components/ui';
    ...
    <div className="st-picker" ref={ref}>
      ...
      {open && search.length >= 1 && (
        <div className="st-picker-list" role="listbox">
          ...
          {!query.loading && (query.data?.items ?? []).length === 0 && <div className="st-picker-option st-muted">Không tìm thấy thuốc.</div>}
          {(query.data?.items ?? []).map((medicine) => (
            ...
            >
              ...
              <span className="st-cell-sub st-nowrap" style={{ color: medicine.available_stock > 0 ? undefined : 'var(--st-danger)' }}>
...
const ExaminationPage = () => {
  ...
  useEffect(() => {
    ...
    let alive = true;
    apiGetDoctorSchedule({ from_date: todayIso(-60), to_date: todayIso(30), page_size: 100 }).then((result) => {
  ...
  const saveRx = async () => {
    const invalid = lines.find(
      (line) => !(Number(line.quantity) > 0) || !line.dosage.trim() || !line.frequency.trim(),
    );
    if (lines.length === 0) {
  ...
  const addMedicine = (medicine: PrescribableMedicine) => {
    ...
    setLines((current) => [
      ...current,
      { key: lineKey++, medicine, quantity: '1', dosage: '', frequency: '', duration_days: '', usage_instructions: '' },
    ]);
    setRxDirty(true);
  };
    ...
    <>
      ...
      <div className="st-grid-main">
        <div className="st-stack">
          <Panel
            title="Bệnh án"
            subtitle={hasRecord ? (recordDirty ? 'Có thay đổi chưa lưu' : 'Đã lưu') : 'Chưa ghi'}
            actions={
              !recordLocked && (
                <Button variant="primary" size="sm" icon={<Save size={14} />} loading={isPending('record')} onClick={saveRecord}>
                  Lưu bệnh án
                </Button>
              )
            }
            ...
            {record.loading && !record.data ? (
              ...
              <div className="st-form-grid">
                <Field label="Triệu chứng" className="st-span-2">
                  {(id) => <textarea id={id} className="st-textarea" disabled={recordLocked} value={recordForm.symptoms} onChange={set('symptoms')} />}
                </Field>
                <Field label="Kết quả thăm khám" className="st-span-2">
                  {(id) => (
                    <textarea id={id} className="st-textarea" disabled={recordLocked} value={recordForm.examination_findings} onChange={set('examination_findings')} />
                  )}
                </Field>
                <Field label="Chẩn đoán" required>
                  {(id) => <input id={id} className="st-input" disabled={recordLocked} value={recordForm.diagnosis} onChange={set('diagnosis')} />}
                </Field>
                <Field label="Mã ICD-10">
                  {(id) => (
                    <input id={id} className="st-input st-mono" disabled={recordLocked} maxLength={20} placeholder="VD: L70.0" value={recordForm.icd10_code} onChange={set('icd10_code')} />
                  )}
                </Field>
                <Field label="Hướng điều trị" className="st-span-2">
                  {(id) => <textarea id={id} className="st-textarea" disabled={recordLocked} value={recordForm.treatment_plan} onChange={set('treatment_plan')} />}
                </Field>
                <Field label="Hẹn tái khám">
                  {(id) => (
                    <input id={id} type="date" min={todayIso()} className="st-input" disabled={recordLocked} value={recordForm.follow_up_date} onChange={set('follow_up_date')} />
                  )}
                </Field>
                <Field label="Dặn dò tái khám">
                  {(id) => <input id={id} className="st-input" disabled={recordLocked} value={recordForm.follow_up_notes} onChange={set('follow_up_notes')} />}
                </Field>
                <Field label="Ghi chú nội bộ của bác sĩ" className="st-span-2" hint="Bệnh nhân không thấy ghi chú này.">
                  {(id) => <textarea id={id} className="st-textarea" disabled={recordLocked} value={recordForm.doctor_notes} onChange={set('doctor_notes')} />}
                </Field>
              </div>
            )}
          </Panel>
          ...
          <Panel
            ...
            actions={
              rxEditable && (
                <>
                  {prescription && (
                    <Button variant="danger" size="sm" icon={<Trash2 size={14} />} disabled={Boolean(pending)} onClick={() => setConfirm('delete-rx')}>
                      Xoá đơn
                    </Button>
                  )}
                  <Button variant="primary" size="sm" icon={<Save size={14} />} loading={isPending('rx')} disabled={lines.length === 0} onClick={saveRx}>
                    Lưu đơn thuốc
                  </Button>
                </>
              )
          ...
          >
            ...
            {lines.length === 0 ? (
              <EmptyState
                ...
                title="Chưa kê thuốc"
                text={rxEditable ? 'Tìm và thêm thuốc ở ô phía trên. Không kê đơn thì cứ hoàn tất khám.' : 'Lượt khám này không có đơn thuốc.'}
              />
            ) : (
                ...
                {lines.map((line) => (
                  <div key={line.key} className="st-line-item">
                    ...
                    <Field label={`SL (${line.medicine.unit_of_measure})`} required>
                      {(id) => (
                        <input id={id} type="number" min={1} className="st-input" disabled={!rxEditable} value={line.quantity} onChange={(e) => updateLine(line.key, { quantity: e.target.value })} />
                      )}
                    </Field>
                    <Field label="Liều dùng" required>
                      {(id) => (
                        <input id={id} className="st-input" placeholder="1 viên" disabled={!rxEditable} value={line.dosage} onChange={(e) => updateLine(line.key, { dosage: e.target.value })} />
                      )}
                    </Field>
                    <Field label="Tần suất" required>
                      {(id) => (
                        <input id={id} className="st-input" placeholder="2 lần/ngày" disabled={!rxEditable} value={line.frequency} onChange={(e) => updateLine(line.key, { frequency: e.target.value })} />
                      )}
                    </Field>
                    <Field label="Số ngày">
                      {(id) => (
                        <input id={id} type="number" min={1} className="st-input" disabled={!rxEditable} value={line.duration_days} onChange={(e) => updateLine(line.key, { duration_days: e.target.value })} />
                      )}
                    </Field>
                    {rxEditable ? (
                      <Button
                    ...
                    <div className="st-span-all" style={{ gridColumn: '1 / -1' }}>
                      <input
                        ...
                        onChange={(e) => updateLine(line.key, { usage_instructions: e.target.value })}
                      />
                    </div>
                  </div>
                ))}
